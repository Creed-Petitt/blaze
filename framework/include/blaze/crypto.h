#ifndef BLAZE_CRYPTO_H
#define BLAZE_CRYPTO_H

#include <string>
#include <string_view>
#include <vector>
#include <boost/json.hpp>
#include <blaze/json.h>
#include <blaze/util/string.h>

namespace blaze::crypto {

    using util::hex_encode;

    [[nodiscard]] std::string sha256(std::string_view input);
    [[nodiscard]] std::string hmac_sha256(std::string_view key, std::string_view data);
    
    // Base64 Utilities
    [[nodiscard]] std::string base64_encode(std::string_view input);
    [[nodiscard]] std::string base64_decode(std::string_view input);
    [[nodiscard]] std::string base64url_encode(std::string_view input);
    [[nodiscard]] std::string base64url_decode(std::string_view input);

    // Random
    [[nodiscard]] std::string random_token(size_t length = 32);

    enum class JwtError {
        None,
        InvalidSignature,
        Expired,
        Malformed
    };

    // JWT
    [[nodiscard]] std::string jwt_sign(const Json& payload, std::string_view secret, int expires_in = 3600);
    [[nodiscard]] Json jwt_verify(std::string_view token, std::string_view secret, JwtError* error = nullptr);

    // Password Hashing (SCRYPT)
    [[nodiscard]] std::string hash_password(std::string_view password);
    [[nodiscard]] bool verify_password(std::string_view password, std::string_view hash);

} // namespace blaze::crypto

namespace blaze {
    using crypto::jwt_sign;
    using crypto::jwt_verify;
    using crypto::JwtError;
}

#endif
